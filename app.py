...
# --- 5. 데이터 연결 및 전처리 (실시간 반영 ttl=0 유지) ---
...
# --- 7. 지능형 챗봇 (💡 겹치지 않는 메뉴 제외 로직 완벽 탑재) ---
if prompt := st.chat_input("가성비 맛집, 텐동 빼고, 추울 때 뭐 먹지?"):
    ...
    with st.chat_message("assistant"):
        ...
        # 💡 [핵심 업데이트] 제외(Negative) 키워드 솎아내기!
        neg_triggers = ["빼고", "빼줘", "말고", "제외", "먹었", "안먹", "싫어", "겹치", "안끌"]
        exclude_words = []
        for trigger in neg_triggers:
            # 트리거 바로 앞 단어 추출 (예: '텐동 먹었거든' -> '텐동')
            matches = re.findall(r'([가-힣0-9a-zA-Z]+)\s*(?:' + trigger + ')', prompt)
            exclude_words.extend(matches)
        
        clean_exclude = []
        for ew in exclude_words:
            # 조사 제거 ('텐동은' -> '텐동')
            for josa in ["은", "는", "이", "가", "을", "를", "랑", "이랑"]:
                if ew.endswith(josa) and len(ew) > len(josa):
                    ew = ew[:-len(josa)]
            if len(ew) >= 1: clean_exclude.append(ew)
            
        # 알아낸 단어는 데이터베이스에서 자비 없이 날려버립니다!
        for ew in clean_exclude:
            res = res[~(
                res['카테고리'].str.contains(ew, na=False) | 
                res['상호명'].str.contains(ew, na=False) | 
                res['메뉴'].str.contains(ew, na=False) | 
                res['특징'].str.contains(ew, na=False)
            )]
            ai_msg = "말씀하신 메뉴는 겹치지 않게 센스있게 빼고 골랐어요! 😉"
            condition_applied = True

        # 제외 키워드가 남아서 긍정 검색을 방해하지 않도록 문장에서 싹 청소
        clean = prompt
        for trigger in neg_triggers:
            clean = re.sub(r'[가-힣0-9a-zA-Z]+\s*' + trigger + r'[가-힣]*', ' ', clean)

        # 나머지 긍정 키워드 정제
        stop_words = ["추천", "알려줘", "맛집", "식당", "메뉴", "음식", "오늘", "점심", "저녁", "집", "곳", "뭐먹지", "어디야", "있는", "곳은", "하기", "좋은", "어때", "어제", "내일"]
