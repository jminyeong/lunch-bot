...
# 🚨 [핵심 에러 방어] 시트에 아직 열이 없어도 강제로 빈 열을 만들어서 에러 원천 차단!
...
# --- 8. 사이드바 (접었다 펼치기 기능 추가!) ---
with st.sidebar:
    # 🚨 [UI 개선] 맛집 데이터베이스를 접었다 펼 수 있는 expander로 변경
    with st.expander("🗂️ 맛집 데이터베이스 열어보기", expanded=False):
        st.caption("동료들의 참여로 업데이트 중!")
        st.dataframe(df[['카테고리', '상호명', '별점', '메뉴']], hide_index=True, use_container_width=True, height=600)
